...
def search_jobs():
    ...
    try:
        ...
        with open("jobs_data.txt", "w", encoding="utf-8") as file:
            for job in jobs_data:
                file.write(f"Название: {job['title']}\n")
                file.write(f"Зарплата: {job['zp']}\n")
                file.write(f"Компания: {job['company']}\n")
                file.write(f"Ссылка: {job['link']}\n")
                file.write("\n")
        print("Данные успешно сохранены в файл 'jobs_data.txt'")
